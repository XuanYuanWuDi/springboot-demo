...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			// 回显数据字典
			selectCeDictLabel: function(datas, value) {
				var actions = [];
				$.each(datas, function(index, dict) {
					if (dict.code == value) {
						var listClass = $.common.equals("default", dict.listClass) ? "" : "badge badge-" + dict.listClass;
						actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, dict.name));
						return false;
					}
				});
				return actions.join('');
    	...
    	modal: {
			...
			openFullNotButton: function (title, url, width, height) {
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					width = 'auto';
					height = 'auto';
			...
			openFullPermissions: function (title, url, width, height) {
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					width = 'auto';
					height = 'auto';
            ...
            // 选卡页方式打开
            openTab: function (title, url, reload) {
            	createMenuItem(url, title, reload);
            },
            // 关闭选项卡
            closeTab: function () {
            	closeItem();
            },
            closeTabReload: function(){
                var dataId = window.frameElement.getAttribute('data-id');
                closeItem(null, dataId);
            },
            // 禁用按钮
            disable: function() {
        ...
        operate: {
			...
			openFull: function(id) {
				...
				$.modal.openFullNotButton("添加" + $.table._option.modalName, url);
			},
			// 添加信息 全屏
			openFull: function(name,url) {
				$.modal.openFullNotButton(name, url);
			},
			// 权限信息 弹框全屏
			openFullUser: function(name,url) {
				$.modal.openFullPermissions(name, url);
			},
			openTab: function(name, url, reload){
			    $.modal.openTab(name, url, reload);
			},
            // 添加访问地址
            addUrl: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
         ...
         // 用户保存并提交权限
            userPerTab: function(url, data, callback) {
            	var config = {
        	        url: url,
        	        type: "post",
        	        dataType: "json",
